...
int main(int argc,char *argv[])
{
  ...
  int namakcount = 0, namaktempcounter = 0;
  for(i=2;;i++)
  {
    //printf("\n%d",i);	  
    ...
    if(i==2)
    {
        ...
        cvSetImageROI(img_tempmatch,cvRect(100,300,200,160));
        IplImage* img_namak = cvCreateImage(cvGetSize(img_tempmatch),8,3);
        cvCopy(img_tempmatch,img_namak,NULL);
     ...
     //  centroidOfImageObjects(img_tempmatch,&pt_katori,&pt_stick, &pt_bhaguna, &pt_kadai,&pt_namak,&pt_bottle,&pt_egg,&pt_ham,&pt_knife);
       template_matching(img_namak,temp_namak,&pt_namak);
...
////////////////////////////For Namak//////////////////////////        
        cvSetImageROI(img,cvRect(100,300,200,160));
        IplImage* img_namak = cvCreateImage(cvGetSize(img),8,3);
        cvCopy(img,img_namak,NULL);
        cvResetImageROI(img);
      //cvShowImage("template image",temp_namak);
      if(namaktempcounter ==0)
      {
        cvSetImageROI(img_namak,cvRect(pt_namak.x - (namak_W)/2,pt_namak.y - (namak_H)/2,namak_W,namak_H));
        cvCopy(img_namak,temp_namak,NULL);
        cvResetImageROI(img_namak);
        namaktempcounter = 1;
     }
	...
	//ham = hamTrack(pt_plate,plate_W,plate_H,temp_ham,img);
     ...
     if(i > 500)
	 namak = namakTrack(pt_namak,namak_W,namak_H,img_namak,temp_namak,&fcnt,&namakcount);		//check speed must be 1 and namakcount declare above
     else
         namak = 0;
	//egg = eggTrack(pt_egg,egg_W,egg_H,img,temp_egg);
	//katori = katoriTrack(&oldpt,&oldx,&oldy,img,temp_katori);	
	//knife = knifeTrack(pt_plate,plate_W,plate_H,temp_plate,img,l,h);
	
	
	//printf("%d\n",knife);

	//fprintf(outFile,"%d %d %d %d %d %d %d\n",bhag,bottl,res,nn,ham,namak,katori);
	//printf("%d %d %d %d %d %d %d\n",bhag,bottl,res,nn,ham,namak,katori);
	
	//if(katori == 1) printf("in use...\n");
	//else printf("not in use...\n");
        ...
        if( namak == 1)
	{
        	//cvPutText(img, " Namak In use", cvPoint(50,50), &font, cvScalar(255,0,0));
		outcome = 1;
	}        	
	else
	{
        	cvPutText(img, "Namak not In use", cvPoint(50,50), &font, cvScalar(255,0,0));
		outcome = 0;	
	}	
