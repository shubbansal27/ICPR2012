...
int knifeTrack(CvPoint pt_plate,int plate_W,int plate_H,IplImage *img_knife,IplImage *img,IplImage *depth_img,int l,int h,IplImage *first_img)
{
	//int result;
	int j;
	//int lz=77,hz=127,ly=133,hy=173;

	IplImage *change_img = cvCreateImage( cvGetSize( depth_img ), IPL_DEPTH_8U, 1);
        IplImage *result_img = cvCreateImage( cvGetSize( depth_img ), IPL_DEPTH_8U, 1);
       
	cvShowImage("depth1",depth_img);
	cvShowImage("first1",first_img);
	...
	cvShowImage("chnage1",change_img);
	...
	//cvShowImage("result_img",result_img);
	...
	int lh = 0,rh = 0;
	IplImage *leftH,*rightH;*/
	/*if(pt.x > 0 && pt.y > 0)
	{
		cvSetImageROI(img,rectl);
		leftH = cvCreateImage(cvGetSize(img),8,3);
		cvCopy(img,leftH,NULL);
        	cvResetImageROI(img);
		lh = 1;
	}
	if(rhpt.x > 0  && rhpt.y > 0)
	{	
		cvSetImageROI(img,rectr);
		rightH = cvCreateImage(cvGetSize(img),8,3);
		cvCopy(img,rightH,NULL);
		cvResetImageROI(img);
		rh = 1;
	}
	
	if(lh == 1)	
		cvShowImage("left",leftH);
	if(rh == 1)	
	cvShowImage("right",rightH);
	
	if(lh == 1)	
		cvReleaseImage(&leftH);
	if(rh == 1)	
		cvReleaseImage(&rightH);*/	

	//cvShowImage("final_result",final_result);	
	//cvReleaseImage(&dst);
